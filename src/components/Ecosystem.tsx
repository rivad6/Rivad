...
function ProjectCard({ title, category, description, icon, colSpan = 1, linkText = "Explorar", href = "#", className, index }: ProjectCardProps) {
  ...
  const LinkContent = (
    <>
      ...
      <div className="relative z-10 h-full flex flex-col justify-between">
        <div className="flex justify-between items-start mb-10 border-b border-brand-accent/30 pb-5">
          <div className="font-mono text-[9px] tracking-[0.4em] text-brand-accent uppercase">
            {category}
          </div>
          <div className="font-mono text-3xl font-bold text-white/5 group-hover:text-brand-accent/20 transition-all duration-500">
            {index}
          </div>
        </div>
        
        <div>
          <h3 className="text-3xl lg:text-5xl font-display font-light mb-6 text-white group-hover:text-brand-accent transition-all duration-500 leading-none">
            {title}
          </h3>
          <p className="text-gray-400 group-hover:text-gray-300 font-sans text-sm md:text-base mb-10 leading-relaxed max-w-sm transition-colors duration-500">
            {description}
          </p>
        </div>

        <div className="inline-flex items-center justify-between w-full text-[10px] font-mono font-bold uppercase tracking-[0.3em] border-t border-brand-accent/30 pt-5 opacity-40 group-hover:opacity-100 group-hover:text-brand-accent text-white transition-all duration-500">
          <span>{linkText}</span>
          <div className="w-8 h-8 rounded-full border border-brand-accent/30 flex items-center justify-center group-hover:border-brand-accent/50 group-hover:bg-brand-accent/10 transition-all shadow-xl">
  ...
  const containerClasses = cn(
    "group relative border-r border-b border-white/10 bg-transparent p-8 md:p-12 hover:bg-white/5 transition-all duration-700 flex flex-col h-full overflow-hidden frosted-layer",
...
export function Ecosystem() {
  ...
  return (
    <section id="proyectos" className="py-24 border-t border-white/10 relative z-20 bg-transparent">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-0 relative z-10 w-full overflow-hidden">
        ...
        >
          <div>
            <h2 className="text-sm font-mono tracking-widest text-brand-accent/60 uppercase mb-4">{t('eco.title')}</h2>
            <h3 className="text-5xl md:text-7xl lg:text-8xl font-display font-light uppercase tracking-tighter leading-[0.95] text-white">
              {t('eco.multiverse')} <br /> 
              <span className="block text-brand-accent font-serif font-black italic tracking-normal text-[0.8em] lowercase ml-8 md:ml-24 mt-2">
                {t('eco.creative')}
              </span>
            </h3>
          </div>
          <p className="text-gray-400 font-serif text-lg md:text-xl font-light max-w-sm hidden lg:block border-l border-brand-accent/30 pl-4 py-2">
            {t('eco.desc')}
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:border-t lg:border-l border-white/10 lg:mx-12 relative z-10">
